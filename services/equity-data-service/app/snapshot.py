...
class SnapshotManager:
    def __init__(self, stocks: Sequence[StockConfig], collector: Callable[[Sequence[StockConfig]], list[Quote]] = collect_quotes) -> None:
        ...
        self.last_refresh_succeeded: bool | None = None
        ...
        async with self._lock:
            ...
            try:
                ...
                if self.snapshot is not None:
                    items = tuple(
                        quote.stale_copy("refresh_failed") if quote.price is not None else quote
                        for quote in self.snapshot.items
                    )
                    self.snapshot = Snapshot(now, items)
                LOGGER.exception("Equity refresh failed")
                return True
            now = datetime.now(timezone.utc)
            previous = {quote.symbol: quote for quote in self.snapshot.items} if self.snapshot else {}
            expected = {stock.symbol for stock in self.stocks}
            indexed: dict[str, Quote] = {}
            for quote in incoming:
                if quote.symbol not in expected:
                    LOGGER.warning("Ignoring unknown collector symbol %s", quote.symbol)
                elif quote.symbol in indexed:
                    LOGGER.warning("Ignoring duplicate collector symbol %s", quote.symbol)
                else:
                    indexed[quote.symbol] = quote
            items: list[Quote] = []
            for stock in self.stocks:
                quote = indexed.get(stock.symbol)
                if quote is None:
                    quote = error_quote(stock, "missing_from_refresh")
                if quote.status == "ok":
                    items.append(quote)
