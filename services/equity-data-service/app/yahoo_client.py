...
def error_quote(stock: StockConfig, category: str) -> Quote:
    """Build a JSON-safe quote without exposing an upstream exception message."""
    return Quote(
        symbol=stock.symbol,
...
def parse_intraday(stock: StockConfig, history: pd.DataFrame,
    ...
    return Quote(
        ...
        as_of=(latest_at + pd.Timedelta(minutes=15)).to_pydatetime(),
...
def fetch_quote(stock: StockConfig, now: datetime | None = None) -> Quote:
    ...
    if daily.latest_close is not None:
        return Quote(
            ...
            exchange=stock.exchange,
            currency=stock.currency,
            price=float(daily.latest_close),
            previous_close=None,
            change=None,
            change_percent=None,
            as_of=None,
            price_date=daily.closing_date,
...
def collect_quotes(stocks: Sequence[StockConfig]) -> list[Quote]:
    ...
    for index, stock in enumerate(stocks):
        ...
        except Exception as exc:  # Strict per-symbol isolation for parsing and future errors.
            category = type(exc).__name__
            LOGGER.error("%s collection failed unexpectedly (%s)", stock.provider_symbol, category)
            quote = error_quote(stock, category)
