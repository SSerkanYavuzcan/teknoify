...
async def _refresh_loop() -> None:
    while True:
        try:
            ...
            LOGGER.exception("Unexpected equity refresh-loop error")
...
async def health() -> dict[str, object]:
    ...
    stale_count = sum(item.status == "stale" for item in snapshot.items)
    error_count = sum(item.status == "error" for item in snapshot.items)
    return {"status": "ok", "ready": True, "lastRefreshAt": iso_utc(manager.last_refresh_at),
            ...
            "staleItemCount": stale_count, "errorItemCount": error_count}
...
async def equity(symbol: str) -> dict[str, object]:
    ...
    snapshot = _snapshot_or_503()
    item = next((item for item in snapshot.items if item.symbol == stock.symbol), None)
    if item is None:
