...
(or null), market-local `priceDate`, data kind, freshness, status, stale state,
and an optional safe error category. Intraday items also include the completed
candle end as UTC `asOf`. Daily bars have `asOf: null`: the service does not
fabricate an exact market-close instant from a date-only upstream bar.
