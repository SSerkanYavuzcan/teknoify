import { logout, requireAuth, startImpersonation } from '../lib/auth.js';
import {
  createProject,
  createUser,
  deleteProject,
...
function renderUsersList(filter = '') {
  ...
  users.forEach((user) => {
    ...
    const impersonateBtn = createEl('button', {
      className: 'btn btn-sm btn-primary',
      text: 'View as'
    });
    impersonateBtn.type = 'button';
    impersonateBtn.disabled = user.role === 'admin';
    impersonateBtn.addEventListener('click', () => {
      const result = startImpersonation(user.id);
      showMessage(
        qs('#users-feedback'),
        result.message || 'Impersonation başlatıldı, dashboard açılıyor.',
        !result.ok
      );
      if (result.ok) {
    ...
    actions.append(editBtn, toggleBtn, impersonateBtn, deleteBtn);
...
async function init() {
  ...
  const session = requireAuth({ role: 'admin', returnTo: '/dashboard/admin.html' });
  if (!session) {
    return;
  }

  initCommonPage({ activePath: '/dashboard/admin.html' });
  renderTabs();
  wireForms();
  renderAll();
