...
function renderUsersList(filter = '') {
  ...
  users.forEach((user) => {
    ...
    const viewAsBtn = createEl('button', {
      className: 'btn btn-sm btn-primary',
      text: 'View as'
    });
    viewAsBtn.type = 'button';
    viewAsBtn.disabled = user.role === 'admin';
    viewAsBtn.addEventListener('click', () => {
      const result = startImpersonation(user.id);
      showMessage(
        qs('#users-feedback'),
        result.message || 'Impersonation başladı. Dashboard açılıyor.',
        !result.ok
      );
      if (result.ok) {
    ...
    actions.append(editBtn, toggleBtn, viewAsBtn, deleteBtn);
