...
## 6. Journey auto-progression (polish, 2026-09-06)

The pinned journey stays scroll-driven; native scroll remains the source of truth. When a scene reaches its authored completion (`SCENE_DONE`: Keşfet 4300 ms, Seç 3600 ms, Bağla 5300 ms after the scene starts, the moment the scripted cursor parks after its last action) the controller waits 900 ms and calls `window.scrollTo({ behavior: 'smooth' })` to the next step's checkpoint, the same target a tab click uses (`section top + ((k+1)/4 + 0.04) × (section height − viewport height)`). Çalıştır never auto-exits. The pending timer lives in the scene's timer list, so leaving the section, a hidden tab or a step change clears it; the advance also re-checks visibility, in-view state and the current step before moving. An `isAutoAdvancing` guard keeps the controller's own scroll from being read as manual navigation. Manual input wins: wheel, touch, scrolling keys after a 900 ms settle window cancel the current scene's advance; a tab click, any pointer press inside the journey, or a step change not caused by the auto scroll disables progression for the rest of that visit (re-armed only after the journey leaves the viewport). Auto-progression is off under reduced motion and on touch devices (`hover: none`), where the journey stays manual. No new loop or dependency: one `setTimeout` per scene plus a safety timeout during the scroll. Real-time scenarios (uninterrupted watch, wheel, keys, tab click, reverse scroll, hidden tab, leaving, reduced motion, touch) are scripted in the session's `jtest/journey.mjs` harness against Edge.
