...
function printMismatchDiagnostics(pair, publicContent, mirrorContent) {
    ...
    console.error(`${pair.serviceArea} page mirror parity check failed.`);
...
function checkMirrorPair(pair) {
    ...
    console.log(
        `${pair.serviceArea} page mirror parity check passed: ${pair.mirrorPagePath} matches ${pair.publicPagePath}.`
    );
    return true;
}
...
function main() {
    try {
        ...
        console.error('Corporate service page mirror parity check failed due to a runtime error.');
